---
title: "Eigenvector WGBS Analysis"
...
for(i in list.files("R", pattern=".R")){source(paste0("R/",i))}
```
...
```
getRegionScores returns the mean eigenvector scores for each input region, as well as Z-score against the background, and a p-value.
